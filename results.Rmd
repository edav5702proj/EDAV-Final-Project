...
```{r, fig.width=1000, fig.height=1200}
dumbo <- all_dsets[["dumbo"]]
# dumbo
head(dumbo)
...
head(dset)
g <- ggplot(dset, aes(as.Date(format(Datetime, format = "%m-%d"), "%m-%d"), avg)) +
    geom_line(color = "grey30") +
    geom_point(size = 0.1) +
    scale_x_date(date_labels = "%b", date_breaks = "1 months", guide = guide_axis(angle = 45)) +
